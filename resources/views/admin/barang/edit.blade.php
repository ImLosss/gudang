                            ...
                            <div class="col-12 col-sm-4">
                                <div class="form-group local-forms">
                                    ...
                                    <input id="test" class="form-control" type="text" name="nama"
                                        value="{{ old('nama', $data->nama) }}" placeholder="Masukan Nama Barang">
                                </div>
                            </div>
                            <div class="col-12 col-sm-4">
                                <div class="form-group local-forms">
                                    ...
                                    <select name="id_merek" id="id_merek">
                                        ...
                                        @foreach ($mereks as $merek)
                                            <option value="{{ $merek->id }}"
                                                {{ $data->id_merek == $merek->id ? 'selected' : '' }}> {{ $merek->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-12 col-sm-4">
                                <div class="form-group local-forms">
                                    ...
                                    <input id="test" class="form-control" type="number" name="stok"
                                        value="{{ old('stok', $data->stok) }}" placeholder="Masukan Nama Barang">
                                </div>
                            </div>
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            // Mendapatkan nilai select #id_merek saat halaman pertama kali dimuat
            var selectedValue = $('#id_merek').val();
            // console.log(id_merek)
            $.ajax({
                url: '/get-type/' + selectedValue,
                type: 'GET',
                success: function(data) {
                    ...
                    data.types.forEach(element => {
                        ...
                        optiontypesHTML = optiontypesHTML +
                            `<option value="${element.id}" ${element.id=={{ $data->id_type }}? 'selected' : ''}>${element.nama}</option>`
